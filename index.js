...
function pushToArray() {
    if (identifier.length) {
        const keyWordMatch = keyWords.findIndex((item) => item.toLowerCase() === identifier.toLowerCase());
        const double = identifierArray.findIndex((item) => item.toLowerCase() === identifier.toLowerCase());
        if (keyWordMatch >= 0) {
